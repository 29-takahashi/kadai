<?php
	...
	function news_index(){
		...
		foreach($results as $row) {
		    ...
		    echo '<img src="img/' .$img. '" alt="' .$alt. '" class="img-responsive center-block">';
	...
	function news_view(){
	    ...
	    if (isset($_GET["id"])){
		    ...
		    foreach($results as $row) {
			    ...
			    echo '<div class="thumbnail"><img class="img-responsive" src="img/' .$img. '"><div class="caption"><h2>' .$news_title. '</h2><p>';
			    echo date('Y/m/d', strtotime($update_date));
			    category_icon($category_id);
			    echo '</p><p class="kiji">' .$news_detail. '</p></div></div>';
			    echo '<div class="text-right"><a href="news.php"><button class="btn btn-default">ニュース一覧</button></a></div>';
			}
			echo '</div>';
	    } 
	    //ニュース記事一覧表示
